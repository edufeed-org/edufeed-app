...
- `discover.contentTypes` — `/discover` tabs a deployment offers (`DISCOVER_CONTENT_TYPES`, subset of `events,learning,articles,boards,communities,people`, default all). Parsed and derived by `$lib/helpers/discover-content-types.js`: the "All" tab is never configured, it appears when ≥2 feed types (events/learning/articles/boards) are enabled and merges exactly those. Disabled types are neither loaded nor linked to (landing badges, dashboard/sidebar CTAs check `isDiscoverTypeEnabled`).
