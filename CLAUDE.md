...
| 39004 | NIP-29     | LiveKit participants — relay-signed "who is in the call" of an AV group (see Groups relay below)      |
...
- `profileSearchRelays` — NIP-50 capable relays for free-text profile search (`PROFILE_SEARCH_RELAYS` env, default Brainstorm's WoT-ranked `wss://tags.brainstorm.world/relay`; `none` disables the remote leg). Read via `getProfileSearchRelays()`; never union with lookup/fallback relays (strfry rejects the `search` filter field). Consumed by `ContactSearchInput`'s `searchProfiles` mode and the impersonation warning. `profileSearchObserver` (`PROFILE_SEARCH_OBSERVER`, npub/hex) is appended as `observer:<hex>` only on relays whose NIP-11 advertises the extension (`getSearchExtensions()` in `relay-search-extensions.js`).
- `trustAssertions.{relays,providers}` — NIP-85 kind 30382 scores (`TRUST_ASSERTION_RELAYS`, `TRUST_ASSERTION_PROVIDERS`; default Brainstorm scores relay + house key, `none` = off). `useTrustScores(getPubkeys)` (`src/lib/stores/trust-scores.svelte.js`, backed by `loaders/trust-assertions.js`, ask-once per pubkey) returns a reactive `Map<pubkey, {rank,hops,followers,muters,reporters}>`. Used to ORDER and BADGE people-picker rows — never to hide anyone: accounts new to Nostr have no score from any provider.
...
**Groups relay:** `GROUPS_RELAYS` (env), read via `getGroupsRelays()` in `relay-helper.js` (`runtimeConfig.appRelays.groups`, no user-override/NIP-65 union — `publishToGroupRelay` writes to that relay only). Default `wss://groups.edufeed.org`, a pyramid-based relay with two behaviors stock relays lack: only whitelisted accounts may create a group (rejection "restricted: only members of this relay can create a group" — surfaced as the friendly `community_groups_relay_membership_required` i18n key via `isRelayMembershipRequired()` in `group-management.js`), and moderation events (9000-9009) older than 60s are rejected ("too old") — `publishToGroupRelay` re-stamps `created_at` and retries once to cover slow NIP-46 bunker approvals. **Live audio/video** follows the NIP-29 AV spec, not NIP-53: a bare `["livekit"]` tag on the group's kind 39000 marks an AV channel (restated on every 9002 — pyramid drops it from an edit that omits it), the client GETs `https://<relay>/.well-known/nip29/livekit/<group-id>` with a NIP-98 header for a LiveKit token (relay probe: 204 on `/.well-known/nip29/livekit`), and the relay publishes kind 39004 with the live participants. All of it lives in `src/lib/groups/livekit.js`, `call-presence*.js`, `group-call.svelte.js` and `components/groups/call/`; the in-call stage is loaded lazily so `livekit-client` never enters a route's static graph. Full guide: `docs/guides/publishing-to-nip29-groups.md`.
...
- **System (OS) notifications:** `src/lib/services/system-notifications.svelte.js` watches the DM and inbox services' reactive getters in a `$effect.root` (started/stopped by the root layout with the account) and fires browser `Notification`s for new known-sender DMs and unread inbox items — foreground only, no service-worker push. Concord channel toasts use the same gate (`shouldToast` in `$lib/helpers/system-notifications.js`) and the same per-device opt-in `appSettings.systemNotificationsEnabled`, toggled on `/settings` via `toggleSystemNotifications()` (`stores/notification-permission.svelte.js`, which asks the browser for permission). Toasts name the sender and what happened, never content. The inbox keeps a standing `pool.group(...).subscription` (`since` = now) beside its one-shot loaders so the bell — and these toasts — see new items live.
