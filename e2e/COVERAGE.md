...
| `groups-pointer.test.js`             | 2                     | Yes  | /groups join-by-address form: garbage input → invalid-pointer toast, stays on /groups (regression net for Chrome's lenient URL parser — percent-encodes forbidden host bytes instead of throwing, so only a real browser exercises the defect); control: well-formed pointer navigates                                                 |
...
| Comments             | Post, reply, delete (the comment box is the shared `ComposerInput` contenteditable; Playwright `fill()` still works on it)                                                                   | Edit comment                                       |
| Composer @mentions   | Covered by Vitest component tests only: `ComposerInput.test.svelte.js` (chips, `@` picker, keyboard, selection API), `MarkdownEditor`, `NoteCreateModal`, `CommentInput`, `ThreadCreateForm` | No e2e for the `@` picker or chip rendering        |
