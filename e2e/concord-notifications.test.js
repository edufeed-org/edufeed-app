 ...
 * PrivateChannelsView auto-selects `channels[0]` (alphabetical, 'de') on
 * mount and marks the active channel read, so with a single channel the row
 * dot clears the instant the view opens and only the area/tab rollup dot
 * would ever be observable. The guest's channel is named to sort SECOND
 * ("Beta Talk" after "Alpha Planung"): the owner opening the Kanäle view
 * auto-selects Alpha, leaving Beta's unread row dot standing until the row
...
test.describe('Concord notifications', () => {
  test('unread dot lights, clears on open, survives reload; reply lights mention pill', async ({
    ...
    // --- owner opens the Kanäle view: Alpha auto-selects (alphabetical
    // first), so Beta's unread survives the mount — its ROW carries the dot
    // while the active Alpha row carries none
