...
async function initializeAccountPersistence() {
  ...
  combineLatest([configReady$, manager.active$]).subscribe(async ([isConfigReady, account]) => {
    ...
    // Auto-migrate from old kind 30382 membership events to kind 30000 follow set
    // One-time migration: if no follow set exists but old relationship events do,
    // create a follow set with those community pubkeys
    ...
    const { getCommunikeyRelays } = await import('$lib/helpers/relay-helper.js');

    // Fetch old kind 30382 events
    const communikeyRelays = getCommunikeyRelays();
    const oldLoader = createTimelineLoader(
      timedPool,
      communikeyRelays,
      { kinds: [30382], authors: [account.pubkey], limit: 100 },
      { eventStore }
    );
    oldLoader().subscribe({
      complete: () => {
        // After loading old events, check if migration is needed
        /** @type {import('rxjs').Subscription | undefined} */
        let migrationSub;
        migrationSub = eventStore
          .replaceable(30000, account.pubkey, 'communities')
          .subscribe(async (followSetEvent) => {
            if (migrationSub) migrationSub.unsubscribe();

            // If follow set already exists, no migration needed
            if (followSetEvent) return;

            // Check for old kind 30382 events
            /** @type {import('rxjs').Subscription | undefined} */
            let oldEventsSub;
            oldEventsSub = eventStore
              .timeline({ kinds: [30382], authors: [account.pubkey] })
              .subscribe(async (oldEvents) => {
                if (oldEventsSub) oldEventsSub.unsubscribe();

                const communityPubkeys = /** @type {string[]} */ (
                  oldEvents
                    .filter((e) => getTagValue(e, 'n') === 'follow')
                ...
                console.log(
                  `🔄 Migrating ${communityPubkeys.length} community memberships from kind 30382 to kind 30000`
                );

                try {
                  const { ensureFollowSetExists } = await import('$lib/helpers/community.js');
                  const { AddUserToFollowSet } = await import('applesauce-actions/actions');
                  const { actionRunner } = await import('$lib/stores/action-runner.svelte.js');

                  // Create follow set with correct d-tag before adding users
                  await ensureFollowSetExists();

                  // Add each community pubkey to the follow set
                  for (const pubkey of communityPubkeys) {
                    await actionRunner.run(AddUserToFollowSet, pubkey, 'communities');
                  }
                  console.log('✅ Community membership migration complete');
                } catch (err) {
                  console.error('❌ Community membership migration failed:', err);
                }
              });
          });
      }
