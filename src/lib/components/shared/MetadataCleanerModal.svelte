...
<script>
  ...
  /** @type {{ open?: boolean, file: File | null, ondone?: (file: File) => void, maxSize?: number | null, mode?: 'review' | 'inspect' }} */
  let {
    open = $bindable(false),
    file = null,
    ondone = () => {},
  ...
  async function runInspect(target) {
    ...
    try {
      ...
      stripOps = strip.ops ?? [];
      // Preselect balanced compression for oversized PDFs so the
      // compression-first layout starts in a state likely to fit the limit.
      compress = oversized && isPdf ? 'balanced' : 'off';
      phase = 'review';
    } catch (e) {
...
{#snippet inspectRemovesList()}
  {#if stripFieldLabels.length > 0}
    <div class="mt-4 rounded-lg bg-base-200 p-2 text-xs">
      <div class="mb-1 font-medium">{m.metaclean_inspect_removes()}</div>
      <ul class="list-inside list-disc font-mono">
        {#each stripFieldLabels as fieldId (fieldId)}
          <li>{fieldId}</li>
        {/each}
  ...
  <dialog class="modal-open modal" use:topLayerModal={handleKeepOriginal}>
    <div class="modal-box max-w-2xl">
      <h3 class="text-lg font-semibold">
        {mode === 'inspect' ? m.metaclean_inspect_title() : m.metaclean_title()}
      </h3>
        ...
        {#if mode === 'inspect'}
          ...
          <div class="modal-action">
            <button
              ...
              data-testid="metaclean-inspect-close"
            >
              {m.metaclean_close()}
            </button>
          </div>
        {:else}
          ...
          {#if oversized}
            <div class="mt-3 alert py-2 alert-warning">
              <span class="text-sm">
                {m.metaclean_oversized_lead({
                  size: formatFileSize(file?.size ?? 0),
                  limit: formatFileSize(maxSize ?? 0)
                })}
              </span>
            </div>
            ...
            <details class="mt-4">
              <summary
                ...
                data-testid="metaclean-details-toggle"
              >
                {m.metaclean_show_metadata()}
              </summary>
              {@render metadataTable()}
