<script>
  // Imports directly from the concord submodules (not the barrel): both
  // community.svelte.js and client.svelte.js have no top-level package
  // imports, so this stays SSR-clean even though the c/[pubkey] community
  // route renders server-side — see community.svelte.js's header comment and
  // Task 7's report for the verified import-path decision.
  import { useConcordCommunity } from '$lib/concord/community.svelte.js';
  import { signerHasNip44 } from '$lib/concord/client.svelte.js';
