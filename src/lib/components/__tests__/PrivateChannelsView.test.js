 ...
 * `isCommunikeyOwner` (communikeyEvent.pubkey === active user, only relevant
 * pre-founding) vs `isConcordOwner` (concord.community.material.owner ===
 * active user, relevant once a community exists — including the standalone
 * `/private/<id>` route, which never has a communikeyEvent at all). See the
 * long comment above these derivations in PrivateChannelsView.svelte.
 */
