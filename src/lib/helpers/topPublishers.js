...
/**
 * Apply the publisher quick-filter selection to an event list. Solo wins:
 * when set, only that author's events show (the hidden list is preserved but
 * ignored). Otherwise hidden authors are removed. Returns the input array
 * unchanged when nothing is selected.
 *
 * @template {{pubkey?: string}} T
 * @param {T[]} events
 * @param {{solo: string | null, hidden: string[]}} selection
 * @returns {T[]}
 */
export function filterEventsByPublisherSelection(events, { solo, hidden }) {
  if (solo) return events.filter((event) => event.pubkey === solo);
