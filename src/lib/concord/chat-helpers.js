...
// The @-mention helpers (spec §5) moved to the shared composer helper; the
// send pipeline (setShortTextContent → tagPubkeyMentions in the pinned dist)
// still turns the inserted reference into a `p` tag at publish time.
// Re-exported so ChannelChat and its tests keep their import path.
export { detectMentionQuery, applyMention } from '$lib/helpers/mention-autocomplete.js';
