...
- **`ambToNostr` `ext` support** (`ambToNostr.ts:434-455`, live in the installed dist): `amb.ext[ns][facet]` where each item is a `Concept` (`{id, type, prefLabel{lang}}`) → `ext:<ns>:<facet>:id`/`:prefLabel:<lang>`/`:type`, or a string → bare `ext:<ns>:<facet>`. Facet keys may contain colons, so `ext.ekw['konfi:zielgruppen']` emits `ext:ekw:konfi:zielgruppen:…` — byte-matching current Konfi output.
...
**B1.** In `convertFormDataToAMB` (`formDataToAmb.js`), build `amb.ext` from the EKW/Konfi form data: `amb.ext = { ekw: { <facet>: [{id, type:'Concept', prefLabel: <all-lang labels>}], …, methodOther: [<string>…], bibleReference: [<string>…], 'konfi:<slug>': [<concepts>], … } }`. Source the multi-language labels from the same `SelectedConcept.labels` the EKW/Konfi pickers already carry (not hardcoded `:de`).
...
**B4. Read stays.** `parseEkwTagsToFormData`/`parseKonfiTagsToFormData` already read prefixed `ext:ekw:` (+ legacy fallback) and handle any `:prefLabel:<lang>`; unchanged. `formDataToEkwTags.js`/`konfiTags.js` emit helpers become unused for the write path — retire them only if no other caller remains (check the preview/tests first; keep the parsers).
...
- **Slice B EKW/Konfi:** `convertFormDataToAMB` builds `amb.ext.ekw.<facet>` / `ext.ekw['konfi:<slug>']`; `ambToNostr` emits `ext:ekw:<facet>:id/:prefLabel:<lang>/:type` for ALL languages; `parseEkwTagsToFormData`/`parseKonfiTagsToFormData` round-trip it; an existing single-`de` resource still reads. Preview now includes EKW/Konfi facets.
