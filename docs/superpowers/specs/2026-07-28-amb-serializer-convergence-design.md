...
> - **Fix:** lift `konfi` out of the facet position into the namespace: `amb.ext['org.edufeed.ekw.konfi'] = { zielgruppen:[…], themen:[…], … }` → `ext:org.edufeed.ekw.konfi:<slug>:id` (`ns=org.edufeed.ekw.konfi` colon-free, `.` legal; `facet=<slug>`; `sub=id`). Non-konfi `ekw` facets (`ext:ekw:gradeLevel:id`) are **already legal** — leave them under `ekw`.
> - **Namespace string DECIDED (2026-07-28): `org.edufeed.ekw.konfi`** (NIP-BOSS full reverse-DNS recommendation, confirmed by the key holder). Implement it as ONE named constant `EKW_KONFI_NS`. The legacy non-konfi `ekw` namespace stays `ekw` (already legal — do NOT reverse-DNS it; that would gratuitously migrate already-conformant gradeLevel/schoolType data).
